...
<font size='4'> Description: Conduct single-omics analysis including univariate
(association test) and multivariate (PCA) analyses on New DIA Plasma and Tissue
Proteomics from AG Krijgsveld (2 datasets) to take an initial look at power of each
dataset in terms of predicting patient cancer recurrences and also potentially to
provide evidence for latter multi-omics analyses. Association test between each
feature (protein level) and cancer recurrence was performed to capture significant
features that can separate recurrence and non-recurrence patients. PCA was performed
to see if there is any significant PC that can separate recurrence and non-recurrence
patients. At the end of this report, summary of all Method Development datasets
is also displayed. </font>
...
Owing to missing values, there might be a situation that only few observations detected
in one sample group, e.g., NUP188 right above, resulting in unreliable t-statistics.
We used probabilistic dropout model (proDA R package) to account for data missingness
...
```
=> MCOLN3 is tested significant, even though no any observation was detected in
non-recurrence patient group. It is because proDA assumes missing values in MS-based
proteomics are more likely to happen in low-abundance area, missing values are inferred
as low protein abundances.
<br/>
<br/>

Assess data power by p-value histogram
```{r}
p <- all_proDAProBaseRes$pval
hist(p, xlab = 'P-value', main = 'Baseline Plasma Proteomics (proDA)')
```
...
PC3 potentially separates recurrence and non-recurrence patients.
```{r}
# Visualize PCs
ggplot(pcTab, aes(x=Recurrence, y=PC3, col=Recurrence, fill=Recurrence)) +
  geom_boxplot(alpha = 0.3, outlier.shape = NA) +
  geom_jitter(position = position_jitter(0.2), size = 2, show.legend = F) +
...
Plot molecular signatures captured by PC3 in input data. We took a look at top
...
topFeats <- pcTopFeatTab$PC3$Feature[1:20]
...
pheatmap::pheatmap(datMatSub,
                   ...
                   color = colorRampPalette(c('navy', 'white', 'red'))(100),
                   cluster_rows = F, cluster_cols = F,
                   scale = 'row', main = 'Baseline plasma proteomics - PC3')
```

Display top features that build the PC3
```{r}
# Display top features with highest loadings of PC of interest
pcTopFeatTab <- dplyr::mutate(pcTopFeatTab$PC3, Gene = plyr::mapvalues(Feature,
                                                                       from = featInfo$Protein.Groups,
                                                                       to = featInfo$Genes,
                                                                       warn_missing = F))
pcTopFeatTab
...
allNAsIdx <- which(apply(SummarizedExperiment::assay(proFolo), 1, function(x) {all(is.na(x))}))
proFolo <- proFolo[-allNAsIdx,]
...
Assess data power by p-value histogram
```{r}
p <- all_proDAProFoloRes$pval
hist(p, xlab = 'P-value', main = 'Follow-up Plasma Proteomics (proDA)')
```
...
PC7 potentially separates recurrence and non-recurrence patients.
```{r}
# Visualize PCs
ggplot(pcTab, aes(x=Recurrence, y=PC7, col=Recurrence, fill=Recurrence)) +
  geom_boxplot(alpha = 0.3, outlier.shape = NA) +
...
Plot molecular signatures captured by PC7 in input data. We took a look at top
20 features with highest loadings and observed protein levels of these top features
...
pheatmap::pheatmap(datMatSub,
                   ...
                   color = colorRampPalette(c('navy', 'white', 'red'))(100),
                   cluster_rows = F, cluster_cols = F,
                   scale = 'row', main = 'Follow-up plasma proteomics - PC7')
```

Display top features that build the PC7
```{r}
# Display top features with highest loadings of PC of interest
...
if (identical(colnames(proBaseMat), colnames(proFoloMat)) &
  ...
  # Remove features fully missing
  allNAsIdx <- which(apply(proPlasmaDiffMat, 1, function(x) {all(is.na(x))}))
  proPlasmaDiffMat <- proPlasmaDiffMat[-allNAsIdx,]
} else {
...
proPlasmaDiffAssoRes <- doSOA(proPlasmaDiff, meta_var = c('Recurrence', 'Gender', 'Age',
                                                          'Smoking', 'Stage', 'Metastasis'),
                              do_onlyPCA = T, pca_method = 'ppca', num_PCs = 17)
proPlasmaDiffAssoRes$tPCASigRes
```
...
pheatmap::pheatmap(datMatSub,
                   ...
                   color = colorRampPalette(c('navy', 'white', 'red'))(100),
                   cluster_rows = F, cluster_cols = F,
                   scale = 'row', main = 'Differential plasma proteomics - PC7')
```
...
# Tissue Proteomics
Based on PCA result obtained by analyzing whole Tissue Proteomics dataset that
includes Tumor and Normal samples, we found that PCA mainly captures source of
variation in tissue conditions, which indicates good data quality. However,
...
Visualize some significant PCs
```{r}
ggplot(pcTab, aes(x=Condition, y=PC1, color=Condition, fill=Condition)) +
  geom_boxplot(alpha = 0.3, outlier.shape = NA) +
  geom_jitter(position = position_jitter(0.2), size = 2, show.legend = F) +
  ggpubr::stat_compare_means(method = 't.test', paired = F,
                             method.args = list(var.equal = T),
...
ggplot(pcTab, aes(x=Recurrence, y=PC9, color=Recurrence, fill=Recurrence)) +
  geom_boxplot(alpha = 0.3, outlier.shape = NA) +
  geom_jitter(position = position_jitter(0.2), size = 2, show.legend = F) +
  scale_color_manual(values=c('#00BFC4', '#F8766D')) +
  scale_fill_manual(values=c('#00BFC4', '#F8766D')) +
...
# Remove features fully missing
allNAsIdx <- which(apply(SummarizedExperiment::assay(proTumor), 1, function(x) {all(is.na(x))}))
proTumor <- proTumor[-allNAsIdx,]
...
dropout model (proDA R package) to account for data missingness and identify differentially
abundant entities that are significantly associated with cancer recurrence. Note
that features in heatmap are ordered by their t-statistics.
```{r}
# Fit linear probabilistic dropout model to normalized data
...
ggplot(topSigFeatData, aes(x=Recurrence, y=Abundance, col=Recurrence, fill=Recurrence)) +
  ...
  th + theme(strip.text = element_text(size = 13, face = 'bold'))
```
=> proDA assumes missing values in MS-based proteomics are more likely to happen
in low-abundance area, so missingness will cause protein abundance distributions
to shift in low-abundance direction to account for uncertainty.
<br/>
<br/>

Assess data power by p-value histogram
```{r}
p <- all_proDAProTumorRes$pval
hist(p, xlab = 'P-value', main = 'Tumor Tissue Proteomics (proDA)')
```
...
# Remove features fully missing
allNAsIdx <- which(apply(SummarizedExperiment::assay(proNormal), 1, function(x) {all(is.na(x))}))
proNormal <- proNormal[-allNAsIdx,]
...
tFeatSigRes$Var1[184] <- 'P09086;P14859'
tFeatSigRes$Gene[184] <- 'POU2F1;POU2F2'
tFeatSigRes$Var1[347] <- 'P0DP23;P0DP24'
tFeatSigRes$Gene[347] <- 'CALM1;CALM2'
tFeatSigRes
tFeatSigRes$Var1[184] <- 'P09086;P14859;Q9UKI9'
tFeatSigRes$Gene[184] <- 'POU2F1;POU2F2;POU2F3'
tFeatSigRes$Var1[347] <- 'P0DP23;P0DP24;P0DP25'
tFeatSigRes$Gene[347] <- 'CALM1;CALM2;CALM3'
...
proDAProNormalRes <- dplyr::select(all_proDAProNormalRes, -c(diff, se, df, avg_abundance, n_approx, n_obs)) %>%
  ...
  dplyr::mutate(Gene = plyr::mapvalues(name,
                                       ...
                                       warn_missing = F))
# Manually shorten names
proDAProNormalRes$name[161] <- 'P09086;P14859'
proDAProNormalRes$Gene[161] <- 'POU2F1;POU2F2'
proDAProNormalRes$name[359] <- 'P0DP23;P0DP24'
proDAProNormalRes$Gene[359] <- 'CALM1;CALM2'
proDAProNormalRes
proDAProNormalRes$name[161] <- 'P09086;P14859;Q9UKI9'
proDAProNormalRes$Gene[161] <- 'POU2F1;POU2F2;POU2F3'
proDAProNormalRes$name[359] <- 'P0DP23;P0DP24;P0DP25'
proDAProNormalRes$Gene[359] <- 'CALM1;CALM2;CALM3'
...
Assess data power by p-value histogram
```{r}
p <- all_proDAProNormalRes$pval
hist(p, xlab = 'P-value', main = 'Normal Tissue Proteomics (proDA)')
```
...
pheatmap::pheatmap(datMatSub,
                   ...
                   color = colorRampPalette(c('navy', 'white', 'red'))(100),
                   cluster_rows = F, cluster_cols = F,
                   scale = 'row', main = 'Normal Tissue proteomics - PC14')
```
...
pheatmap::pheatmap(datMatSub,
                   ...
                   color = colorRampPalette(c('navy', 'white', 'red'))(100),
                   cluster_rows = F, cluster_cols = F,
                   scale = 'row', main = 'Differential tissue proteomics - PC9')
```
...
```{r}
# Load and subset all Method Development datasets except DIA Tissue Proteomics from
# AG Krijgsveld
...
subPlasma <- function(summ_exp) {
  ...
  # Remove features fully missing
  allNAsIdx <- which(apply(assay(base), 1, function(x) {all(is.na(x))}))
  if (length(allNAsIdx) != 0) {
    base <- base[-allNAsIdx,]
  }
  allNAsIdx <- which(apply(assay(folo), 1, function(x) {all(is.na(x))}))
  if (length(allNAsIdx) != 0) {
    folo <- folo[-allNAsIdx,]
  }
...
subTissue <- function(summ_exp) {
  ...
  # Remove features fully missing
  allNAsIdx <- which(apply(assay(tumor), 1, function(x) {all(is.na(x))}))
  if (length(allNAsIdx) != 0) {
    tumor <- tumor[-allNAsIdx,]
  }
  allNAsIdx <- which(apply(assay(normal), 1, function(x) {all(is.na(x))}))
  if (length(allNAsIdx) != 0) {
    normal <- normal[-allNAsIdx,]
  }
...
Visualize Recurrence-related features from New DIA Plasma and Tissue Proteomics
generated by AG Krijgsveld through volcano plots. Log2-FC (up and down) is recurrence
to non-recurrence groups. To be clearer, entities on up side are more abundant in
recurrence group than non-recurrence group, and so forth. For t-test between recurrence
and non-recurrence groups, proDA is engaged.
```{r message = F}
# Load normalized data
proPlasma <- readRDS('./data/AG_Krijgsveld/new_proPlasmaNorm_DIA.rds')
proTissue <- readRDS('./data/AG_Krijgsveld/new_proTissueNorm_DIA.rds')
...
Log2-FC cutoffs are 1.25 (FC=2.38) and -1.25 (FC=0.42).
```{r}
# TTP
volTab <- proTumorVolTab
volTab['DE'] <- 'Not significant'
volTab$DE[volTab$pVal <= 0.05 & volTab$logFC > 1.25] <- 'Up'
volTab$DE[volTab$pVal <= 0.05 & volTab$logFC < -1.25] <- 'Down'
volTab['Label'] <- NA
label4SigFeats <- volTab$Gene[volTab$DE != 'Not significant']
volTab$Label[volTab$DE != 'Not significant'] <- label4SigFeats
...
ggplot(volTab, aes(x=logFC, y=-log10(pVal), col=DE, label=Label)) +
  geom_point(size = 2) +
  geom_hline(yintercept = -log10(0.05), col = 'red', linewidth = 0.4) +
  geom_vline(xintercept = c(-1.25, 1.25), col = 'red', linewidth = 0.4) +
  geom_text_repel(size = 4, show.legend = F) +
  scale_color_manual(values = c('navy', 'grey', 'red')) +
...
and non-recurrence patient groups for all Method Development datasets we currently
have, including Targeted/Untargeted Plasma/Tissue Metabolomics and Lipidomics (6
datasets) and DIA/DDA Plasma/Tissue Proteomics (6 datasets). Numbers above bars
indicate total feature numbers.
```{r message = F}
...
new_all_proDAproNormalRes_Kri_DIA <- proDA::test_diff(fit,
                                                      contrast = Intercept + `smpAnno$RecurrenceYes` - Intercept,
                                                      sort_by = 'pval')
new_proDAproNormalRes_Kri_DIA <- dplyr::filter(new_all_proDAproNormalRes_Kri_DIA, pval < 0.05)
```
...
```
=> Plasma samples in general perform better than Tissue samples and Baseline samples
perform as good as Follow-up samples in terms of separating recurrence and non-recurrence
patient groups. Noticeably, DIA Normal Tissue Proteomics performs nicely, indicating
that tumor microenvironment may be critical to cancer recurrence.
