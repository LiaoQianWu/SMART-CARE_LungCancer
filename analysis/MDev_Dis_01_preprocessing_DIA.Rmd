...
was then stored in SummarizedExperiment objects for further analyses. At end, feature
correlations and result consistency (Tumor vs Normal samples) between datasets before
and after data adjustment is investigated.

For Klin. data: Sample **Q89YQK_NG**, **I0HVOL_TU**, **7EAOX7_TU**, **PDIR55_NG**,
**1TK71I_TU**, **NG1IWD_TU**, **125CII_TU**, and **1LJXIN_TU** were excluded. (Check
Section '**Show reasons for removing samples**' for more details)

For Krij. data: Sample **I0HVOL_TU**, **7EAOX7_TU**, **PDIR55_NG**, **3M0D40_TU**,
**MJMTYR_TU**, and **XFKHGP_NG** were excluded.
...
# Save preprocessed data containing all samples except Q89YQK_NG
# tmp_proTissueTab <- dplyr::filter(allProTissueTab, !Sample %in% 'Q89YQK_NG')
# proTissuePrepro <- doPreprocessing(tmp_proTissueTab, feat = 'Feature', smp = 'Sample',
#                                    val = 'Abundance', smpAnno = c(smpAnno, 'MissLevel'),
#                                    featAnno = 'Gene', do_featFilt = T, cutoff = 0.5, viz_miss = T,
#                                    save_path = './data/Discovery/AG_Klingmueller/allProTissue')
# rm(tmp_proTissueTab)
...
# Do PCA
pcaRes <- doSOA(allProTissueVsn, meta_var = 'Condition', pca_method = 'ppca', num_PCs = 20, do_onlyPCA = T)
# Show significant associations between PCs and sample conditions
pcaRes$pcSigAssoRes
# Visualize PCs
pcTab <- pcaRes$pcTab %>%
...
# subProTissueVsn <- doPreprocessing(subProTissueTab, feat = 'Feature', smp = 'Sample',
#                                    val = 'Abundance', smpAnno = c(smpAnno, 'MissLevel'),
#                                    featAnno = 'Gene', do_featFilt = T, cutoff = 0.5)$vsn.data
# soaRes <- doSOA(subProTissueVsn, meta_var = 'Condition', pca_method = 'ppca',
#                 num_PCs = 20, use_proDA = T, level_metaVar = c('Tumor', 'Normal'))
# saveRDS(soaRes, './data/Discovery/AG_Klingmueller/soaRes/proTissueRes_signature_rmBadSmps.rds')
soaRes <- readRDS('./data/Discovery/AG_Klingmueller/soaRes/proTissueRes_signature_rmBadSmps.rds')
...
# Save preprocessed data containing all samples
# proTissue_Combined <- doPreprocessing(allProTissueTab, feat = 'Protein.Group', smp = 'Sample',
#                                       val = 'Abundance', featAnno = c('Protein.Names', 'Genes'),
#                                       smpAnno = c(smpAnno, 'MissLevel'), do_featFilt = T, cutoff = 0.5, viz_miss = T,
#                                       save_path = './data/Discovery/AG_Krijgsveld/combined_allProTissue')
...
datMat_BC <- doBatchCorrection(proTissue_Combined$vsn.data,
                               wantedVar = c('Condition', 'Recurrence', 'Condition:Recurrence'),
                               unwantedVar = 'Cohort')
...
# Do batch correction
datMat_BC <- doBatchCorrection(allProTissueVsn,
                               wantedVar = c('Condition', 'Recurrence', 'Condition:Recurrence'),
                               unwantedVar = 'Cohort')
allProTissueVsn_BC <- allProTissueVsn
assay(allProTissueVsn_BC) <- datMat_BC
# Save batch corrected data
# saveRDS(allProTissueVsn_BC, './data/Discovery/AG_Krijgsveld/combined_allProTissueVsnBC.rds')
...
# subProTissueVsn <- doPreprocessing(subProTissueTab, feat = 'Protein.Group', smp = 'Sample',
#                                    val = 'Abundance', featAnno = c('Protein.Names', 'Genes'),
#                                    smpAnno = c(smpAnno, 'MissLevel'), do_featFilt = T, cutoff = 0.5)$vsn.data
# soaRes <- doSOA(subProTissueVsn, meta_var = 'Condition', pca_method = 'ppca', num_PCs = 20,
#                 use_proDA = T, unwantVar = 'Cohort', level_metaVar = c('Tumor', 'Normal'))
# saveRDS(soaRes, './data/Discovery/AG_Krijgsveld/soaRes/combined_proTissueRes_signature_rmBadSmps.rds')
soaRes <- readRDS('./data/Discovery/AG_Krijgsveld/soaRes/combined_proTissueRes_signature_rmBadSmps.rds')
topSigFeatIdx <- order(abs(soaRes$featSigAssoRes$Stat), decreasing = T)[1:100]
topSigFeats <- soaRes$featSigAssoRes$Var1[topSigFeatIdx]
# Order top significant features by t-statistics
topSigFeatStats <- soaRes$featSigAssoRes$Stat[topSigFeatIdx]
...
# Prepare batch-corrected abundance matrix of all samples
datMat_BC <- assay(allProTissueVsn_BC)
# Cluster samples by sample conditions
...
# Prepare data with constructed SVs for statistical testing and data corrected for
# SVs for computing feature correlations
svaRes_New <- doSVA(proTissue_New, wantedVar = c('Condition', 'Recurrence', 'Condition:Recurrence'))
svaRes_Old <- doSVA(proTissue_Old, wantedVar = c('Condition', 'Recurrence', 'Condition:Recurrence'))
...
# saveRDS(condiFeatAssoTabs, './data/Discovery/AG_Klingmueller/soaRes/comparisons/New_vs_Old_condiFeatAsso.rds')
condiFeatAssoTabs <- readRDS('./data/Discovery/AG_Klingmueller/soaRes/comparisons/New_vs_Old_condiFeatAsso.rds')
...
#                                               unwantVar1 = grep('^SV\\d+$', colnames(colData(proTissueSVs_New)), value = T),
#                                               unwantVar2 = grep('^SV\\d+$', colnames(colData(proTissueSVs_Old)), value = T))
# saveRDS(condiFeatAssoTabs_SVs, './data/Discovery/AG_Klingmueller/soaRes/comparisons/New_vs_Old_condiFeatAsso_SVs.rds')
condiFeatAssoTabs_SVs <- readRDS('./data/Discovery/AG_Klingmueller/soaRes/comparisons/New_vs_Old_condiFeatAsso_SVs.rds')
...
# Prepare data with constructed SVs for statistical testing and data corrected for
# SVs for computing feature correlations
svaRes_Klin <- doSVA(proTissue_Klin, wantedVar = c('Condition', 'Recurrence', 'Condition:Recurrence'))
svaRes_Krij <- doSVA(proTissue_Krij, wantedVar = c('Condition', 'Recurrence', 'Condition:Recurrence'))
proTissueSVs_Klin <- svaRes_Klin$summExp_SVs
proTissueSVs_Krij <- svaRes_Krij$summExp_SVs
# Prepare protein-gene table for providing gene names
...
<font size='4'> **Preprocessed data without any covariate correction** </font>\
**Feature correlations (data1: Klin. data, data2: Krij. data)**
```{r}
# Compute feature correlations between two datasets
corrRes <- calcFeatCorr(proTissue_Klin, proTissue_Krij, show_nFeats = T)
# Plot feature correlations
ggplot(corrRes, aes(x=Correlation)) +
  geom_histogram(binwidth = 0.1) +
  scale_x_continuous(breaks = c(seq(-1, 1, 0.2))) +
  labs(y = 'Frequency', title = 'Feature Correlations: Klin. vs Krij. Proteomics (Uncorrected)') +
  th
  # theme(axis.title = element_text(size = 22),
  #       axis.text = element_text(size = 18))
```
...
# Compute log(FC) and t-statistics between Tumor and Normal tissue samples in two datasets
# Prepare summarized tables of comparison for visualization
# condiFeatAssoTabs <- summarizeFeatAssoRes(proTissue_Klin, proTissue_Krij, 'Klin.', 'Krij.',
#                                           meta_var = 'Condition', subset = NULL,
#                                           method = 'proDA', alpha = 0.05, prot_gene_tbl = featInfo)
# saveRDS(condiFeatAssoTabs, './data/Discovery/AG_Klingmueller/soaRes/comparisons/newKlin_vs_Krij_condiFeatAsso.rds')
condiFeatAssoTabs <- readRDS('./data/Discovery/AG_Klingmueller/soaRes/comparisons/newKlin_vs_Krij_condiFeatAsso.rds')
...
```{r}
# Compute feature correlations between two datasets
corrRes <- calcFeatCorr(svaRes_Klin$correctDatMat, svaRes_Krij$correctDatMat, show_nFeats = F)
# Plot feature correlations
ggplot(corrRes, aes(x=Correlation)) +
  geom_histogram(binwidth = 0.1) +
  scale_x_continuous(breaks = c(seq(-1, 1, 0.1))) +
  labs(y = 'Frequency', title = 'Feature Correlations: Klin. vs Krij. Proteomics (Corrected)') +
  th
```
...
# Compute log(FC) and t-statistics between Tumor and Normal tissue samples in two datasets
# Prepare summarized tables of comparison for visualization
# condiFeatAssoTabs_SVs <- summarizeFeatAssoRes(proTissueSVs_Klin, proTissueSVs_Krij,
#                                               'Klin.', 'Krij.', meta_var = 'Condition',
#                                               subset = NULL, method = 'proDA',
#                                               alpha = 0.05, prot_gene_tbl = featInfo,
#                                               unwantVar1 = grep('^SV\\d+$', colnames(colData(proTissueSVs_Klin)), value = T),
#                                               unwantVar2 = grep('^SV\\d+$', colnames(colData(proTissueSVs_Krij)), value = T))
# saveRDS(condiFeatAssoTabs_SVs, './data/Discovery/AG_Klingmueller/soaRes/comparisons/newKlin_vs_Krij_condiFeatAsso_SVs.rds')
condiFeatAssoTabs_SVs <- readRDS('./data/Discovery/AG_Klingmueller/soaRes/comparisons/newKlin_vs_Krij_condiFeatAsso_SVs.rds')
...
ggplot(condiFeatAssoTabs_SVs$tbl_pVal, aes(x=-log10(pVal_dat1), y=-log10(pVal_dat2), col=Significance)) +
  ...
  th
```
=> Feature significance of New Klin. data is not as good as that of Krij. or Old Klin.
data while sample spaces are identical.
