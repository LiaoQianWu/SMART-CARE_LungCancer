...
**1LJXIN_TU**, **NG1IWD_TU** were removed due to misclassification in metadata-assisted
quality control (Tumor vs Normal, PC1), whose "Tumor Purity (%)||Missing Level (%)"
is 30||51, 10||41, 40||27, and 25||28.
...
# Remove Sample 'Q89YQK_NG' due to high level of missingness (71%) and potential
# outlier according to PC2
# Remove some misclassified Tumor samples with low tumor purity
...
proTissuePrepro <- doPreprocessing(proTissueTab, feat = 'Feature', smp = 'Sample', val = 'Abundance',
                                   ...
                                   do_featFilt = T, cutoff = 0.5, viz_miss = T, save_path = './data/Discovery/AG_Klingmueller/proTissue')
...
```{r}
# Do PCA
pcaRes <- doSOA(proTissuePrepro$vsn.data, meta_var = c('Condition', 'TumorPurity', 'MissLevel'),
...
ggplot(pcTab, aes(x=Condition, y=`PC1 (33.7%)`, col=Condition, fill=Condition, label=Label)) +
...
# ggplot(pcTab, aes(x=`PC1 (33.7%)`, y=`PC2 (14.5%)`, col=Condition, shape=Extra)) +
#   geom_point(size = 2.5) +
#   scale_color_brewer(palette = 'Dark2') +
...
# ggplot(pcTab, aes(x=`PC1 (33.7%)`, y=`PC2 (14.5%)`, col=TumorPurity, shape=Condition)) +
#   geom_point(size = 2.5) +
#   scale_color_continuous(name = 'Tumor purity (%)') +
#   th

# Check missing levels of samples
ggplot(pcTab, aes(x=`PC1 (33.7%)`, y=`PC2 (14.5%)`, col=MissLevel*100, shape=Condition)) +
  geom_point(size = 2.5) +
  scale_color_continuous(name = 'Missing level (%)', type = 'viridis') +
  th

# Display tumor purity and missing levels of misclassified samples
badSmpInfo <- dplyr::distinct(proTissueTab, Sample, .keep_all = T) %>%
  dplyr::select(Sample, TumorPurity, MissLevel) %>%
  dplyr::filter(Sample %in% c('I0HVOL_TU', '7EAOX7_TU'))
badSmpInfo
```
...
<font size='4'> **Check data reproducibility: New vs Old Proteomics sharing partial samples** </font>\
...
proTissue_Old <- readRDS('./data/MethodDev/AG_Klingmueller/proTissueVsnBC.rds')
...
ggplot(corrRes, aes(x=Correlation)) +
  ...
  labs(y = 'Frequency', title = 'Feature correlations: New vs Old Proteomics') +
  th
```
...
# Keep only common samples shared between two datasets
cmnSmps <- intersect(colnames(proTissue_New), colnames(proTissue_Old))
proTissue_New <- proTissue_New[, cmnSmps]
proTissue_Old <- proTissue_Old[, cmnSmps]
...
ggplot(condiFeatAssoTabs$tbl_logFC, aes(x=logFC_dat1, y=logFC_dat2, col=Direction)) +
  ...
  labs(x = 'log(FC) of New data', y = 'log(FC) of Old data',
       title = 'Tumor vs Normal: DIA Tissue Proteomics') +
  th

# Show percentage of consistent results
percent <- round(sum(condiFeatAssoTabs$tbl_logFC$Direction != 'Inconsistent') /
                   nrow(condiFeatAssoTabs$tbl_logFC), 3) * 100
