...
proTissueTab <- readr::read_tsv('./data/AG_Krijgsveld/20230815_MarcS_Discovery_Tissue.pg_matrix.tsv') %>%
  ...
  # Remove these samples due to bad quality
  dplyr::filter(!Sample %in% c('I0HVOL_TU', '7EAOX7_TU', 'MJMTYR_TU', 'XFKHGP_TU'))
...
```
=> Samples 'I0HVOL_TU', '7EAOX7_TU', 'MJMTYR_TU', and 'XFKHGP_TU' were removed due
to bad quality. Boxplot displaying separation between Normal and Tumor tissues is
shown at end of this section.
<br/>
<br/>

Show dimensions of original data (82 samples and 7986 features)
```{r}
...
Show dimensions of filtered data (82 samples and 5381 features)
```{r}
...
Show data quality
```{r}
# Load old vsn normalized data
tmp_proTissueVsn <- readRDS('./data/AG_Krijgsveld/proTissueVsn4ShowQuality.rds')
# Do single-omics analysis
proTissueSOA <- doSOA(tmp_proTissueVsn, meta_var = 'Condition', pca_method = 'ppca',
                      do_onlyPCA = T)
...
ggplot(pcTab, aes(x=Condition, y=PC1, col=Condition, fill=Condition, label=Label)) +
